...
// Habilita el middleware para analizar el cuerpo de la solicitud en formato JSON
...
app.put('/update-user', async (req, res) => {
    ...
    if (!userId || !name || !email) {
        return res.status(400).json({ error: 'ID de usuario, nombre y correo son obligatorios' });
    }
    ...
    try {
        ...
        // Actualizar la base de datos con los nuevos datos del usuario
        const { error: updateError } = await supabase
            .from('users')
            .update(updatedData)
            .eq('id', userId);
